...
| `attribute_capture.py` | **Stage 3.** From a real BD/CPP capture (`cov_emit_links.json`), build the watch set as the **union of the Black Duck BoM, a Claude reconstruction from the compiled file paths, and `.git` discovery**, enumerate each candidate repo's tree, and attribute every compiled file to repos via the mapping service (multi-map). Records the **provenance triple** (actual source / ground truth / fallback) and a **Claude-resolved `watchRef`** (moving branch) per repo; tags each file with an `origin` linking mirrored copies. Monitored = owns ≥1 *primary* TU; linked-only (OpenSSL) → reference-only. | BD API + `cov_emit_links.json` | `live-stage3/build-capture.json`, `live-stage3/hub-api-components.json` | BD + Claude + GitHub |
| `repo_mapper.py` | The compiled-file→repo **attribution service** (`attribute(compiled_paths, repo_filesets) -> {path: [Attribution,…]}`). Fat batch interface (so a future impl can hand the whole set to a frontier LLM). **Multi-map**: longest-suffix owner(s) + a directory-level *vendored-cluster* signal, so a file that is both built directly and inline-vendored fans out to the host **and** upstream (both watched) while a lone shared basename does not. | — (library) | — | none |
